...
1. **Yeni kod bu kurala uyar.** Makine kapısı yalnız **eklenen satırları** tarar ve orada yalnız **dosyadaki sayısı artan** Türkçe adı bulgu sayar; bu yüzden eski adlar günlük işi kırmızıya boyamaz. Bir adı çeviren PR, aynı satırda duran eski adlar yüzünden kırılmaz (tüketici bildirimi #88, 2026-09-23). Sayısı artan ad üç sınıfa ayrılır (tüketici bildirimi #100, 2026-09-24):
   - **Kırmızı:** depoda tabanda hiç bulunmayan Türkçe ad, ya da var olan Türkçe adın **yeni tanımı** (`const`/`let`/`function`/`class`/`interface`/`type`/`enum`, parametre, yapı bozma).
   - **Uyarı (engellemez):** tabanda zaten tanımlayıcı olarak duran bir adın **kullanımı**: içe aktarma, alan okuma, nesne anahtarı, çağrı. Raporda görünür; çevirisi projenin kendi teknik borç kaydında yapılır.
   - SQL dosyalarında sınıflama yapılmaz; sayısı artan her ad kırmızıdır.
...
| **GitHub akışı** | `.github/workflows/kod-dili.yml` (görevli) + `ornek/kod-dili.yml` (projeye kopyalanan tek dosya) | Her PR'da ve main gönderiminde **eklenen satırları** tarar; yeni Türkçe ad ya da var olan Türkçe adın yeni tanımı varsa kırılır. Var olan adın kullanımı uyarıdır. |
...
- **Ekran yazısı ayıklayıcısı yalnız `.tsx` / `.jsx` dosyalarında çalışır** (2026-09-24, #102). Eskiden her dosyada çalışıyordu ve `=` ya da sonda `;` taşımayan satırı yazı sayıp atıyordu. Noktalı virgülsüz yazılan projelerde bu, gerçek kod demekti: 11 projede 2.511 gerçek ad görünmüyordu (Gunum-Var 1.148, SNN-Yonetici-Ozeti 1.153). Düzeltmeden sonra tam taramanın sayıları bu kadar artar. PR kapısı etkilenmez; o adlar tabanda zaten var, yalnız uyarı verirler. `.tsx` dosyalarında aynı sezgi hâlâ çalışır.
- Satır başındaki düzenli ifade, **önceki satırın son işaretine** göre tanınır (`=`, `,`, `(`, `:`, `?`, `||`, `;`, `{`, `return` …). Tanımlayıcı ya da `)` ile biten satırdan sonra gelen `/` bölmedir ve sonrası taranır.
- Blok yorumun `*` ile başlamayan orta satırları hâlâ taranır. 2026-09-24'te 11 projede ölçüldü: 1 satır (Gunum-Var `edge-abacus.characterization.test.ts:101`).
